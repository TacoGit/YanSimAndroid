...
public class DepthOfFieldScatter : PostEffectsBase
{
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		Camera component = GetComponent<Camera>();
		focalDistance01 = ((!focalTransform) ? FocalDistance01(focalLength) : (component.WorldToViewportPoint(focalTransform.position).z / component.farClipPlane));
		...
		else if (blurType == BlurType.DX11 && (bool)dx11bokehMaterial)
		{
			if (highResolution)
			{
				...
				dx11bokehMaterial.SetVector("_Screen", new Vector3(1f / (float)source.width, 1f / (float)source.height, internalBlurWidth));
			...
			else
			{
				...
				dx11bokehMaterial.SetBuffer("pointBuffer", cbPoints);
				dx11bokehMaterial.SetTexture("_MainTex", dx11BokehTexture);
				dx11bokehMaterial.SetVector("_Screen", new Vector3(1f / (float)renderTexture2.width, 1f / (float)renderTexture2.height, internalBlurWidth));
