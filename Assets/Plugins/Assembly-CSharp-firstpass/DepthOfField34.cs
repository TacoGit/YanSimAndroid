...
public class DepthOfField34 : PostEffectsBase
{
	...
	private const float BOKEH_EXTRA_BLUR = 2f;
	...
	private float FocalDistance01(float worldDist)
	{
		...
		return component.WorldToViewportPoint((worldDist - component.nearClipPlane) * component.transform.forward + component.transform.position).z / (component.farClipPlane - component.nearClipPlane);
	}
	...
	//	float num = ((!bokeh) ? 1f : 2f);
	//	bool flag = quality > Dof34QualitySetting.OnlyBackground;
	//	Camera component = GetComponent<Camera>();
	//	float num2 = focalSize / (component.farClipPlane - component.nearClipPlane);
	//	if (simpleTweakMode)
	//	{
	//		focalDistance01 = ((!objectFocus) ? FocalDistance01(focalPoint) : (component.WorldToViewportPoint(objectFocus.position).z / component.farClipPlane));
	//		focalStartCurve = focalDistance01 * smoothness;
	//		focalEndCurve = focalStartCurve;
	//		flag = flag && focalPoint > component.nearClipPlane + Mathf.Epsilon;
	//	}
	//	else
	//	{
	//		if ((bool)objectFocus)
	//		{
	//			Vector3 vector = component.WorldToViewportPoint(objectFocus.position);
	//			vector.z /= component.farClipPlane;
	...
	//		flag = flag && focalPoint > component.nearClipPlane + Mathf.Epsilon;
	//	}
	//	widthOverHeight = (float)source.width / (float)source.height;
	...
	//	dofMaterial.SetVector("_InvRenderTargetSize", new Vector4(1f / (float)source.width, 1f / (float)source.height, 0f, 0f));
	...
	private void Downsample(RenderTexture from, RenderTexture to)
	{
		...
		Graphics.Blit(from, to, dofMaterial, 6);
	}
	...
	private void AddBokeh(RenderTexture bokehInfo, RenderTexture tempTex, RenderTexture finalTarget)
	{
		...
		num2 += bokehScale * maxBlurSpread * 2f * oneOverBaseSize;
