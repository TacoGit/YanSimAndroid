...
public class Bloom : PostEffectsBase
{
	...
	//	BloomScreenBlendMode bloomScreenBlendMode = (doHdr ? BloomScreenBlendMode.Add : screenBlendMode);
	...
	//			if (quality > BloomQuality.Cheap)
	//			{
	//				blurAndFlaresMaterial.SetVector("_Offsets", new Vector4(0f, 1.5f / (float)temporary.height, 0f, 0f));
	//				Graphics.SetRenderTarget(temporary);
	//				GL.Clear(false, true, Color.black);
	//				Graphics.Blit(temporary5, temporary, blurAndFlaresMaterial, 4);
	//				blurAndFlaresMaterial.SetVector("_Offsets", new Vector4(1.5f / (float)temporary.width, 0f, 0f, 0f));
	...
	//		else
	//		{
	//			float num4 = Mathf.Cos(flareRotation);
	//			float num5 = Mathf.Sin(flareRotation);
	//			float num6 = hollyStretchWidth * 1f / num * num2;
	//			float num7 = hollyStretchWidth * num2;
