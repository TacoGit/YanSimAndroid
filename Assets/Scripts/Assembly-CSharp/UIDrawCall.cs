...
public class UIDrawCall : MonoBehaviour
{
	...
	private void CreateMaterial()
	{
		...
		} else if (mMaterial != null) // DONT USE NULL SHADERS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		{
	...
	public void UpdateGeometry(int widgetCount)
	{
		...
		if (count > 0 && count == uvs.Count && count == cols.Count && count % 4 == 0)
		{
			...
			if (count < 65000)
			{
				...
				mMesh.SetUVs(1, (uv2.Count != count) ? null : uv2);
				mMesh.SetNormals((norms.Count != count) ? null : norms);
				mMesh.SetTangents((tans.Count != count) ? null : tans);
				if (flag)
				{
	...
	private void Awake()
	{
		if (dx9BugWorkaround == -1)
		{
			...
			dx9BugWorkaround = ((platform == RuntimePlatform.WindowsPlayer && SystemInfo.graphicsShaderLevel < 40 && SystemInfo.graphicsDeviceVersion.Contains("Direct3D")) ? 1 : 0);
		}
		if (ClipRange == null)
		{
			ClipRange = new int[4]
