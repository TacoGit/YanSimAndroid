	...
	private void Update()
	{
		...
		if (Prompt.Circle[0] != null && Prompt.Circle[0].fillAmount == 0f)
		{
			...
			if (!Yandere.Chased && Yandere.Chasers == 0)
			{
				...
				{
					Yandere.Character.GetComponent<Animation>().CrossFade("f02_bucketDrop_00");
		...
		if (Yandere.Pouring)
		{
			if (PourHeight == "Low" && Input.GetButtonDown("B") && Prompt.DistanceSqr < 1f)
			{
