...
public class PoseModeScript : MonoBehaviour
{
	...
	private void Update()
	{
			...
			else if (Customizing)
			{
				...
				else if (Selected == 2)
				{
					...
					if (InputManager.TappedLeft)
					{
						...
						{
							Student.Cosmetic.Accessory = ((!Student.Male) ? (Student.Cosmetic.FemaleAccessories.Length - 1) : (Student.Cosmetic.MaleAccessories.Length - 1));
						}
						Student.Cosmetic.Start();
						UpdateLabels();
					}
		...
		else
		{
			...
			if (Placing && Input.GetButtonDown("A"))
			{
	...
	public void UpdateLabels()
	{
		...
		if (ChoosingAction)
		{
			...
			OptionLabels[7].text = "Release Student";
			Limit = 7;
			if (Student.Male)
			{
		...
		else if (ChoosingBodyRegion)
		{
			...
			uILabel2.color = new Color(uILabel2.color.r, uILabel2.color.g, uILabel2.color.b, (!Student.Male) ? 1f : 0.5f);
		}
		else if (ChoosingBone)
		{
