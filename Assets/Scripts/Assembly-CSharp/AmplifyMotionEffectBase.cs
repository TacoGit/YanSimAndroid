...
public class AmplifyMotionEffectBase : MonoBehaviour
{
	...
	private void OnPostRender()
	{
		...
		float num = ((!(DepthThreshold > float.Epsilon)) ? float.MaxValue : (1f / DepthThreshold));
		...
		if (flag)
		{
			float num2 = ((!(m_deltaTime >= float.Epsilon)) ? 0f : (MotionScale * CameraMotionMult * (1f / m_deltaTime)));
			float scale2 = (m_starting ? 0f : num2);
			m_motionRT.DiscardContents();
