...
public class MissionModeMenuScript : MonoBehaviour
{
	...
	private void Start()
	{
		...
		CustomPopulationLabel.text = string.Empty;
	...
	private void Update()
	{
		...
		if (Phase == 1)
		{
			...
			if (Speed > 1f)
			{
				...
				if (Speed > 3f)
				{
					if (!InfoSpoke[0])
					{
						component.PlayOneShot(InfoLines[0]);
						InfoSpoke[0] = true;
					}
		...
		else if (Phase == 2)
		{
			...
			{
				component.PlayOneShot(InfoLines[Selected]);
		...
		else if (Phase == 3)
		{
			...
			{
				Transform transform9 = Objectives[l];
				transform9.localScale = Vector3.Lerp(transform9.localScale, (l <= Difficulty) ? Vector3.one : Vector3.zero, Time.deltaTime * 10f);
			}
		...
		else if (Phase == 4)
		{
			...
			Jukebox.GetComponent<AudioSource>().volume -= Time.deltaTime;
		...
		else if (Phase == 5)
		{
			...
			if (Input.GetButtonDown("A"))
			{
				...
				if (PromptBar.Label[0].text == "Toggle")
				{
		...
		else
		{
			...
			if (!Input.anyKey)
			{
				...
				else if (MissionIDString[0] != '-')
				{
					GetNumbers();
					bool flag = false;
					if ((TargetNumber > 9 && TargetNumber < 21) || TargetNumber > 97)
					{
						flag = true;
					}
					if (TargetNumber == 0)
			...
			{
				if (ErrorLabel.text == "Valid Mission ID!")
				{
	...
	private void ChooseTarget()
	{
		if (Phase != 5)
		{
			...
			if (TargetNumber > 5 && TargetNumber < 21)
			{
				ChooseTarget();
			}
		}
		...
		string url = "file:///" + Application.streamingAssetsPath + "/Portraits/Student_" + TargetID + ".png";
		WWW wWW = new WWW(url);
		if (TargetNumber > 5 && TargetNumber < 21)
		{
	...
	private void UpdateNemesisDifficulty()
	{
		...
		else
		{
			...
			NemesisPortrait.mainTexture = ((NemesisDifficulty <= 2) ? NemesisGraphic : BlankPortrait);
		}
	}
	...
	public void UpdateGraphics()
	{
		...
		{
			string url = "file:///" + Application.streamingAssetsPath + "/Portraits/Student_" + MissionModeGlobals.MissionTarget + ".png";
			WWW wWW = new WWW(url);
	...
	private void UpdateObjectiveHighlight()
	{
		...
		PromptBar.Label[2].text = (((Column != 1 || Row >= 5) && (Column != 2 || Row != 6)) ? string.Empty : "Change");
	...
	{
		TargetString = ((TargetID >= 10) ? string.Empty : "0") + TargetID;
		if (CustomObjectives[2].alpha == 1f)
		{
			WeaponString = ((RequiredWeaponID >= 10) ? string.Empty : "0") + RequiredWeaponID;
		}
		else
		{
			WeaponString = "00";
		}
		ClothingString = ((CustomObjectives[3].alpha != 1f) ? "0" : RequiredClothingID.ToString());
		DisposalString = ((CustomObjectives[4].alpha != 1f) ? "0" : RequiredDisposalID.ToString());
		for (int i = 1; i < CustomObjectives.Length; i++)
		{
			if (CustomObjectives[i] != null)
			{
				ConditionString[i] = ((CustomObjectives[i].alpha != 1f) ? "0" : "1");
			}
		}
	...
	{
		GetComponent<AudioSource>().PlayOneShot(InfoLines[6]);
		Globals.DeleteAll();
