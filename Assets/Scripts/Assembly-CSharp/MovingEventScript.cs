...
public class MovingEventScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if ((Clock.HourTime > 13.375f && !Poisoned) || EventStudent.Dying || EventStudent.Splashed)
		{
			EndEvent();
			return;
		}
		...
		else if (EventPhase == 2)
		{
			...
			if (Timer > 2f)
			{
				if (Yandere.Inventory.ChemicalPoison || Yandere.Inventory.LethalPoison)
				{
	...
	private void EndEvent()
	{
		if (!EventOver)
		{
			...
			Animation component = EventStudent.Character.GetComponent<Animation>();
			component[EventStudent.BentoAnim].weight = 0f;
