...
public class JsonScript : MonoBehaviour
{
	...
	private void Start()
	{
		...
		else if (SceneManager.GetActiveScene().name == "CreditsScene") credits = CreditJson.LoadFromJson(CreditJson.FilePath);
	}
	...
	private void ReplaceDeadTeachers(string[] firstNames, string[] lastNames)
	{
		...
		for (int j = 90; j < 101; j++)
		{
			if (StudentGlobals.GetStudentReplaced(j))
			{
				...
				if (j == 97) studentJson.Accessory = "7";
				if (j == 90) studentJson.Accessory = "8";
			}
		}
	}
}
