...
public class CustomizationScript : MonoBehaviour
{
	...
	private void Start()
	{
		...
		ApologyWindow.localPosition = new Vector3(1360f, ApologyWindow.localPosition.y, ApologyWindow.localPosition.z);
	...
	private void Update()
	{
		...
		else if (Phase == 3)
		{
			...
			if (CustomizePanel.alpha == 1f)
			{
				...
				if (tappedDown || tappedUp)
				{
					...
					{
						Selected = ((Selected > 1) ? (Selected - 1) : 6);
					}
		...
		if (ApologyWindow.localPosition.x < -1360f)
		{
			ApologyWindow.localPosition = new Vector3(1360f, ApologyWindow.localPosition.y, ApologyWindow.localPosition.z);
			Apologize = false;
			Timer = 0f;
